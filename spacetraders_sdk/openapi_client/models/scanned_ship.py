...
class ScannedShip(BaseModel):
    """
    The ship that was scanned. Details include information about the ship that could be detected by the scanner.  # noqa: E501
    """

    symbol: StrictStr = Field(default=..., description="The globally unique identifier of the ship.")
    registration: ShipRegistration = Field(...)
    nav: ShipNav = Field(...)
    frame: Optional[ScannedShipFrame] = None
    reactor: Optional[ScannedShipReactor] = None
    engine: ScannedShipEngine = Field(...)
    mounts: Optional[conlist(ScannedShipMountsInner)] = Field(
        default=None, description="List of mounts installed in the ship."
    )
    __properties = ["symbol", "registration", "nav", "frame", "reactor", "engine", "mounts"]
    ...
    @classmethod
    def from_json(cls, json_str: str) -> ScannedShip:
    ...
    @classmethod
    def from_dict(cls, obj: dict) -> ScannedShip:
        """Create an instance of ScannedShip from a dict"""
        if obj is None:
            return None

        if not isinstance(obj, dict):
            return ScannedShip.parse_obj(obj)

        _obj = ScannedShip.parse_obj(
            {
                ...
                "mounts": (
                    [ScannedShipMountsInner.from_dict(_item) for _item in obj.get("mounts")]
